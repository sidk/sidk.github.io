---
id: 344
title: Saving Multiple Records with One Form
date: 2016-05-24T16:10:45-04:00
author: Sid
layout: revision
guid: http://ducktypelabs.com/338-revision-v1/
permalink: /338-revision-v1/
---
Forms which post data to a `#create` or `#update` action are concerned with one particular resource. For example, a form to update a user&#8217;s profile. The sequence looks something like this:
...
Though some percentage of the forms in our app can be described this simply, often the forms we find ourselves needing to build are not that simple to describe. They might need to save multiple records, or more than one table, and/or perform some additional processing (like sending an email), or there is more than one context under which a record can be updated (for example a multi-step application form).
...
A form object is a Plain Old Ruby Object (PORO). It takes over from the controller wherever it needs to talk to the database and other parts of your app like mailers, service objects and so on. A form object typically functions together with a dedicated controller.
...
Now, while there are good reasons to go with either option 1 or 2, I&#8217;m going to elaborate on option 3. There are a few benefits to using a form object in a situation like this:
  ...
  3. Your view will likely be simpler to write (read: no nested forms) and contain less logic.
