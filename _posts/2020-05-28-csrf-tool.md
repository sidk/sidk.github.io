...
2. Create a form (within an `iframe` to prevent a redirect) with `action` and `method` set to the specified endpoint and HTTP method, and submit it using the default content type.
